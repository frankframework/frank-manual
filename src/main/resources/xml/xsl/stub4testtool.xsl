...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	...
	<xsl:template match="*|@*|comment()|processing-instruction()">
		<xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="parent::*[name()='pipe']">
						...
						<xsl:choose>
							...
							<xsl:when test="@className='nl.nn.adapterframework.http.HttpSender'">
								<xsl:call-template name="copy" />
							</xsl:when>
							<xsl:when test="@className='nl.nn.adapterframework.ldap.LdapSender'">
								<xsl:call-template name="copy" />
							</xsl:when>
							<xsl:otherwise>
			...
			<xsl:when test="name()='pipe' and (@className='nl.nn.adapterframework.ftp.FtpFileRetrieverPipe' or @className='nl.nn.adapterframework.extensions.tibco.SendTibcoMessage')">
				<xsl:element name="pipe">
					<xsl:attribute name="name">
						<xsl:value-of select="@name"/>
					</xsl:attribute>
