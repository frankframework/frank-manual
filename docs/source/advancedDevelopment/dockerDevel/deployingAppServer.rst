...
.. NOTE::
   ...
   It is possible to include ``resources.yml`` in the Docker image. In this case, all external resource descriptions for all DTAP stages have to be included. When the customer changes the external resources to be used, a new deployment of the product is needed. In most cases this is not a good way of working.