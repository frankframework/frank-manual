...
The Frank!Framework sometimes suspects that an error storage or message log is missing when this is not really the case. In the exercise of the previous section, there was an adapter that called sub-adapters. The Frank!Framework complained that these sub-adapters did not have error storages. This was no problem however, because the parent adapter did have an error storage. The Frank!Framework did not take into account that the sub-adapters were only called from this parent adapter.
