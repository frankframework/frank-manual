...
**Answer:** The warning means what it says. HTTP requests can contain parameters that are provided via the URL in query parameters (the part after the ``?`` in the URL). It is a security risk if the Frank!Framework uses all query parameters supplied in a request. A malicious caller would have options to try hacking by adding unexpected parameters to HTTP requests. You have a few options here. First, it can be that your Frank application needs all query parameters the caller may provide. In that case you can suppress the warning. Search the warning in the logfile to find instructions for suppressing. Second, it can be that your adapter should not use any query parameters. Then set attribute ``allowAllParams="false"`` on your ``<ApiListener>``. Third, it can be that you know what query parameters you want to use. Then list them in attribute ``allowedParameters``. In this case you do not have to set ``allowAllParams="false"`` -- the Frank!Framework changes the default value automatically.
