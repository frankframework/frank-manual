...
def makeMarkdown(w, markupLanguage):
    ...
    lines, err = unindentAndCheck([unTab(l) for l in w.getLines()])
    ...
    header = "```" + markupLanguage
    if len(emphasizeNums) >= 1:
        header = header + "{" + ", ".join([str(n) for n in emphasizeNums]) + "}"
    footer = "```"
    return [header] + lines + [footer], None
